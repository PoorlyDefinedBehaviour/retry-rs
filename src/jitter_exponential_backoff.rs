...
struct JitterExponentialBackoff {
  pub jitter_factor: f32,
  pub exponent: u32,
  pub wait_for: u32,
}

impl JitterExponentialBackoff {
  /// The backoff time will be wait_for^2.
...
#[async_trait]
impl crate::retry::Backoff for JitterExponentialBackoff {
  async fn wait(&mut self, _retry: usize) {
    tokio::time::sleep(Duration::from_secs(self.wait_for as u64)).await;
    self.wait_for = self.wait_for.pow(self.exponent) + (rand::thread_rng());
  }
}
...
mod tests {
  ...
  fn binary_initial_wait_time_must_be_greater_than_1() {
    let _ = JitterExponentialBackoff::binary(0);
  }
  ...
  async fn smoke() {
    ...
    let result: Result<i32, &str> = Retry::new()
      .retries(3)
      .backoff(JitterExponentialBackoff::binary(2))
      .exec(|| {
        tries += 1;
